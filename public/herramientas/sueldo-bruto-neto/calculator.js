...
// Valores vigentes feb-dic 2026. La UMA 2026 entra en vigor el 1-feb-2026.
const UMA_D=117.31, SUB_LIM=11492.66, SUB_M=536.22;
...
function isrBeforeSubsidy(g){const b=tarifa.find(x=>g>=x[0]&&g<=x[1])||tarifa[tarifa.length-1];return Math.max(0,b[2]+(g-b[0])*b[3])}
// Aproximación de cuota obrera IMSS para trabajador subordinado: prestaciones en dinero,
// gastos médicos pensionados, invalidez y vida, cesantía/vejez y excedente de 3 UMA.
...
function fromGross(g){const rawISR=isrBeforeSubsidy(g);const subsidyEntitlement=g<=SUB_LIM?SUB_M:0;const subsidyApplied=Math.min(rawISR,subsidyEntitlement);const finalISR=Math.max(0,rawISR-subsidyApplied);const m=imss(g);return{gross:g,isr:finalISR,imss:m,subsidy:subsidyApplied,net:g-finalISR-m}}
